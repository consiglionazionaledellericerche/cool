...
public class BulkInfoCoolSerializer extends BulkInfoSerializer {
    ...
    private String propJSONCMISValue(FieldProperty fieldProperty, BulkInfoCool bulkInfo, CmisObject cmisObject) {
        ...
        if (propertyName != null && !propertyName.equals("")) {
            ...
            if (propertyName.equalsIgnoreCase(PolicyType.ADD_REMOVE_ASPECT_REQ_PARAMETER_NAME) && value == null) {
                return Optional.ofNullable(fieldProperty.getOuter())
                        .map(BulkInfo::getCmisTypeName)
                        .filter(s -> cmisObject.<List>getPropertyValue(PropertyIds.SECONDARY_OBJECT_TYPE_IDS).contains(s))
                        .map(s -> "add-".concat(s))
                        .orElse("");
            }
    ...
    private String getMultiValue(CmisObject cmisObject, String propertyName) {
        // TODO test
        String multiReturn = "[";
        Iterable valueList = cmisObject.getPropertyValue(propertyName);
    ...
    private String getValueByPropertyType(CmisObject cmisObject,
        ...
        if (property.getPropertyType().value().equals("boolean")) {
            ...
            BigDecimal bdValue = cmisObject.getPropertyValue(propertyName);
